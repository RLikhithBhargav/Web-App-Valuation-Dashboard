...
def update_valuation_chart(selected_label, data):
    ...
    x_labels = [f"Col {i+1}" for i in range(len(numeric_row))]
    df = pd.DataFrame({"Column": x_labels, "Value": numeric_row})
    fig = px.bar(df, x="Column", y="Value", title=f"{selected_label} Over Columns 1..n")
